            ...
            "result": [
                {
                    ...
                    "timestamp": [int(datetime.now(UTC).timestamp())] * len(closes),
...
def test_succeeds_on_second_attempt(mock_get, mock_sleep):
    mock_get.side_effect = [httpx.ReadTimeout("timeout"), _resp([42.0])]
    provider = YahooFinanceProvider()
...
@patch("app.core.http.time.sleep")
@patch("app.market_data.yahoo_finance_provider.httpx.get")
def test_empty_close_after_filtering_fails_without_retry(mock_get, mock_sleep):
...
def test_missing_currency_is_not_invented(mock_get, mock_sleep):
    ...
    with pytest.raises(MarketDataError, match="ValueError"):
